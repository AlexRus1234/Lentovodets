...
- Legacy `nil-backup` cartridges are deliberately not read
- `filetape` emulator: a regular file behaves like a tape — development
...
bind = "127.0.0.1:29201"     # loopback = not visible from the network
                              # (SSH tunnel); a LAN address is a deliberate
                              # choice and requires a password
...
Direct LAN access is a deliberate operator choice (changed via `bind`);
in that case the daemon requires a configured password and refuses to
start without one.
...
Fresh clone: run `make web-build` first (the bundle is embedded via
