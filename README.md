...
- [Права доступа и развёртывание](#права-доступа-и-развёртывание)
...
- Инкрементальные бекапы: новые и изменённые файлы определяются по
  размеру, mtime и xxhash64; symbolic и hard links сохраняются,
  специальные файлы (FIFO, сокеты, устройства) пропускаются со статистикой
- Многотомные бекапы с продолжением по кассетам (spanning): сессия режется
  ...
  каталогов — поддерево целиком на одной кассете; смена кассет —
  интерактивный промпт CLI или диалог демона (`awaiting_tape`)
...
- Верификация после записи (`--verify`): записанные сессии сразу
  перечитываются, xxhash каждого файла сверяется с индексом (~×2 к времени
  записи, по умолчанию выключено; полная диагностика носителя — `readtest`)
...
- **Full** — вся кассета подряд, включая следование цепочке spanning
  (смена кассеты со сверкой ярлыков и обратных ссылок цепочки);
  повреждённые сессии пропускаются с переходом к следующим
- **Selective** — выбранные пути из конкретной сессии (Web UI); при
  сессии-части цепочки подсказывает, какую кассету вставить
...
- Восстановление в безопасный каталог (`--dest`) или по исходным путям
  (`--original`); `--dest` не выпускает записи за пределы каталога
  назначения (пути с `..` и обход через восстановленные symlink
  отклоняются)
...
- Поиск по подстроке, фильтрация по кассете, удаление сессий, prune
  устаревших
- История версий файла: все копии пути с датой, кассетой, сессией и
  хешем (`catalog copies`, Web UI) — любую версию можно восстановить
  выборочно
- `catalog rebuild` — реконструкция каталога из JSON-индексов вставленной
  кассеты (DR: утерян `lentovodec.db`, кассета вне каталога, переезд на
  новую машину); идемпотентно, tar-поток не читается
...
- Демон: фоновые задачи бекапа/восстановления (одна активная — стример
  один), REST API, graceful shutdown; задача на границе кассеты
...
> Релизные бинарники (`lentovodets-<version>-linux-amd64` + `.sha256`)
> публикуются вручную из CI в Packages и Releases Forgejo; для разработки —
...
Новой (чистой) кассете ничего не нужно — `tape format` создаёт ярлык на
пустой ленте. Кассете, ранее записанной чужим софтом (LTFS, tar/dd без
Лентоводца), — нужна: `tape format` / `tape info` падают ошибкой чтения
блока (`input/output error`). Запишите один filemark в начало ленты:

```bash
mt-st -f /dev/nst0 rewind && mt-st -f /dev/nst0 weof && mt-st -f /dev/nst0 offline
```

`weof` помечает ленту как пустую: старые данные физически **не
стираются**, но после перезаписи ярлыка Лентоводцем становятся
недостижимыми. Команда пишет на ленту — нужен доступ к устройству
(группа `tape`); утилита на Arch — `mt-st` (пакет `mt-st`), на прочих
дистрибутивах — `mt`. Диагностика и другие типовые проблемы реального
железа — в [`docs/func/ru/os-setup.md`](docs/func/ru/os-setup.md),
...
session_ttl = "72h"
webhook_url = ""              # POST-уведомление о завершении задач демона;
                              # пусто — выключено
webhook_timeout = "10s"       # таймаут доставки (сбой — один повтор)
