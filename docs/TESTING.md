...
| E2E (Web UI)  | `tests/e2e/specs/*.spec.ts`        | —         | демона на filetape через REST и браузер (Playwright; opt-in CI-шаг `run_e2e_tests`) |
...
- Не делаем снапшот-тесты UI. Тонкие smoke-проверки — вручную; ключевые
  сценарии Web UI (дерево файлов сессии, выбор и восстановление) —
  Playwright E2E в `tests/e2e`: global-setup собирает бинарь с web-бандлом,
  поднимает демона на filetape-ленте во временном каталоге и сеет данные
  через REST (format → job → full backup). Локально — `make test-e2e`,
  в CI — opt-in галочка `run_e2e_tests`.
