...
`Num = 1`, `Type = FULL`, `Part = k` (канон раскладки — FORMAT §3.1).

### 2.7. `TapeLabel`

См. [FORMAT §4](FORMAT.md#4-tapelabel-json). Канонические поля: `Magic`,
...
Ограничения и миграции схемы:

- `CHECK`-ограничения на `type` и `state`; `UNIQUE (tape_uuid,
  session_num)` исключает дублирование номера сессии на одной ленте.
...
`GetLatestFileStates` выполняет один запрос для всего набора путей сессии
(важно для производительности `mirror`).
...
Заполнение кассет в среднем ниже, чем при резке по файлам; это компромисс
в пользу локальности: поддерево целиком на одной кассете,
...
закрывается JSON-блоком-указателем (FORMAT §5.1). `TapeFullError`
...
контента (перезапись соседних дорожек и ошибки серводанных обнаруживаются
только повторным чтением), поэтому опционально сразу после записи часть
перечитывается:
после фиксации в каталоге (`SaveFiles`) и до плановой смены кассеты —
`Rewind + MTFSF(2K−1)` → `ReadSession(dest=nil)`: декодер сверяет
xxhash каждого файла против индекса, use case сверяет счётчики состава с
...
записи (то же чтение на той же скорости); по умолчанию выключено.
**Политика ошибки:** откат не выполняется — данные уже на ленте и в
каталоге, перезапись не устраняет дефект носителя. Любой сбой верификации —
...
части уже восстановлены. Предельный вариант DR — без Лентоводца: каждая
кассета цепочки читается GNU tar (рецепт `mt`/`dd`/`tar` —
FORMAT §11).
...
4. Повреждение данных копии (`SessionDamageError`, несовпавший хеш) —
   следующая копия (честный fallback на более старую). Прочие ошибки
   чтения (запись в dest, транспортный сбой, отмена ctx) перебором
   копий не лечатся — немедленный возврат исходной ошибки: иначе сбой
   записи в dest маскировался бы под «все копии повреждены».
5. Если ни одна копия не прочиталась → `ErrNoHealthyCopy`.

Позиционирование сессий детерминировано (см. [FORMAT §8](FORMAT.md#8-правила-перемотки)).
...
- `Rebuild()` — реконструкция каталога из содержимого вставленной кассеты
  ...
  [FORMAT §8](FORMAT.md#8-правила-перемотки)) → `CreateSession` +
  ...
  цепочке без оператора (auto-changer) не реализовано. Отчёт:
...
- `ReadTest()` — диагностическое чтение всей ленты с проверкой хешей без
  ...
  в отличие от restore, — ошибка (диагностика носителя должна сообщать
  об ошибках, а не пропускать их).
...
HTTP API (с аутентификацией, см. §6 и §9.2). Граница режимов явная.
...
аутентификации — один из двух способов (модель двух ключей):
...
текстом «нет кассеты в приводе». Probe статуса считает устройство,
занятое задачей, доступным: оно открыто владельцем, а ожидание
освобождения заняло бы часы.
...
ожидание — задача завершается ошибкой отмены, не ожидая 30 с. Отдельный
...
доступу администратора. Список разрешённых корней не вводится. Ответ сортирует
...
bind = "127.0.0.1:29201"     # что слушать; 127.0.0.1 — LAN не видит порт
                             # (SSH-туннель см. §9.2); LAN-адрес требует пароля
...
| Отмена операций    | `context.Context` во всех use case; демон отменяет задачу по SIGINT |
...
- Привилегированный helper (polkit/udisks-style) — рассмотрен и
  **отвергнут**: одна машина, один стример; усложнение не оправдано.

Инструкция развёртывания (`useradd`, udev, systemd-юнит с харднингом) —
в [`docs/func/ru/os-setup.md`](func/ru/os-setup.md). Кодовых изменений
модель не требует: доступ к
...
- **`bind = "127.0.0.1:29201"`** (дефолт). Управление с другой машины —
  через SSH-туннель, аутентификация и шифрование обеспечиваются SSH:
...
Модель аутентификации (рассчитана на одного администратора):
...
- **Сессии** — случайные 256 бит (`crypto/rand`), in-memory map с TTL
  (`session_ttl`, дефолт 72h) и лимитом размера; JWT и подписи не
  используются — отзыв всех сессий выполняется перезапуском демона.
...
- HTTP внутри LAN без TLS — принятый компромисс для локальной сети:
  токен передаётся в её пределах; при необходимости TLS используется
  обратный прокси или SSH-туннель, демон не работает с сертификатами.

Ограничения модели: пользователь один, ролей нет;
сессии не переживают перезапуск демона; WebSocket/SSE-эндпоинтов нет
(прогресс — поллинг), токен не передаётся в долгоживущие
соединения.
