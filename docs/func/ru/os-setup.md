...
(`0001-Use-Arch-Linux-device-access-groups.patch`): ленточные устройства
получают группу `storage`, а группа `tape` в поставке отсутствует.
Типичная проблема первой установки на Arch: демон стартует, но probe падает
...
выглядит как «правило не сработало», хотя оно применяется с небольшой
задержкой.
...
**Прямой доступ из LAN** включается изменением `bind`
...
`web_username` — отказ старта с диагностическим сообщением. HTTP без TLS —
принятый компромисс для локальной сети; при необходимости TLS —
обратный прокси или SSH-туннель.
...
## Особенности Arch Linux (проблемы первой установки)
...
3. **`mt` — пакет `mt-st`, только в AUR** (`yay -S mt-st`; в базовой
   поставке утилиты нет), исполняемый файл называется `mt-st`, а не `mt`:
