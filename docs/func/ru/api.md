...
- Неаутентифицированный запрос → `401 {"error": "unauthorized", "code":
  "auth_required"}`.
- Сессия — случайные 256 бит из `crypto/rand`, хранится в памяти демона
  (in-memory map с TTL `session_ttl` из конфига, по умолчанию 72h, лимит
  1024). JWT и подписи не используются; отзыв всех сессий выполняется
  перезапуском демона.
...
- Header-based аутентификация выбрана вместо cookie сознательно: CSRF
  исключается (сторонняя страница не может установить произвольный
  заголовок).
...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/api/tape/info` | Прочитать `TapeLabel` текущей кассеты и активные TapeAlert-флаги привода |
...
операции немедленно возвращают `409 {"code": "task_running"}`, без
ожидания освобождения устройства. Привод без кассеты (ENOMEDIUM) — `409
{"error": "нет кассеты в
приводе", "code": "no_medium"}` вместо сырого errno.

Поле `alerts` содержит объекты `{name, code, critical}`. При недоступной
диагностике возвращается пустой массив, а ярлык остаётся доступен.
...
Для `POST /api/restore/start` параметр `session_id` включает selective-восстановление из этой сессии. Если `session_id` задан без `paths`, восстанавливаются все записи выбранной сессии; с `paths` — на диск пишутся только указанные пути и их поддеревья (остальные записи сессии читаются и проверяются по хешам, но не распаковываются). При `dest` пути с `..` в индексе ленты отклоняются, запись через восстановленные symlink запрещена — восстановление не выходит за каталог назначения.
