...
│   └── vite.config.ts
├── docs/                      # этот каталог
├── legacy/                    # старый nil-backup (после Этапа 0)
├── .gitignore
├── .editorconfig
├── .golangci.yaml
├── Makefile
...
Двойной EOF после ярлыка — это стандартный способ обозначить EOD на ленте
(см. [FORMAT](FORMAT.md#физическая-раскладка)).
...
Web UI требуют запущенного `lentovodec daemon`. Это компромисс между
«всегда через демон» (старый TUI так и пытался, но местами ходил в engine
напрямую) и «всегда локально» (тогда Web невозможен).
...
`.yaml`/`.json` (см. проблему в legacy, docs/LEGACY_REFERENCE.md).
...
- Дублирование типов (как `JobConfig` в legacy в двух местах).
