...
Завершённые этапы дорожной карты. До 2026-09-20 велись в
...
формулировки этапов («Файлы»/«Тесты»/«Готовность», статусные цитаты) —
как в исходном плане.

Разделение с корневым [CHANGELOG.md](../CHANGELOG.md): CHANGELOG —
пользовательские релизные заметки «что изменилось» (формат Keep a
Changelog, перевод — [CHANGELOG.EN.md](../CHANGELOG.EN.md)); этот
файл — история работ по этапам дорожной карты (милстоуны,
зафиксированные решения «в рамках свободы impl.», отступления от
плана, развёрнутые результаты). Текущие планы и направления —
[ROADMAP.md](ROADMAP.md); ручные проверки перед публикацией релиза —
