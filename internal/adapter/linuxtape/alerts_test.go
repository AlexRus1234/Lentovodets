...
func tapeAlertParam(code int, value ...byte) []byte {
	...
	binary.BigEndian.PutUint16(param[:2], uint16(0x800+code))
...
func TestParseTapeAlertPage_ActiveAndUnknown(t *testing.T) {
	data := tapeAlertResponse(
		...
		tapeAlertParam(2, 0x00, 0x02),
		tapeAlertParam(42, 0x01),
		tapeAlertParam(3, 0x00),
	)
	got, err := parseTapeAlertPage(data)
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if len(got) != 3 || got[0].Name != "read-warning" || !got[1].Critical || got[2].Name != "alert-0x2A" {
